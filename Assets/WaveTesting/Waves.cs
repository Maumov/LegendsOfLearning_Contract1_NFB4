...
public class Waves : MonoBehaviour
{
    ...
    public float GetHeight(Vector3 position)
    {
        ...
        //weighted sum
        var height = MeshFilter.mesh.vertices[index(p1.x, p1.z)].y * (max - Vector3.Distance(p1, localPos))
                   + MeshFilter.mesh.vertices[index(p2.x, p2.z)].y * (max - Vector3.Distance(p2, localPos))
                   + MeshFilter.mesh.vertices[index(p3.x, p3.z)].y * (max - Vector3.Distance(p3, localPos))
                   + MeshFilter.mesh.vertices[index(p4.x, p4.z)].y * (max - Vector3.Distance(p4, localPos));
